...
#' library(datasets)
#' library(RJSONIO)
#' df <- data.frame(name = cardata$X, cylinders = cardata$cylinders)
#' # Make the json 
#' cylindercount <- as.character(unique(df$cylinders))
#' cyl <- lapply(1:length(cylindercount), function(y){
#' list(name=paste0("data.cylinder.", cylindercount[y], sep = ""), 
#' size = 3938,imports = c("",""))})
